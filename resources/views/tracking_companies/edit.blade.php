                ...
                <div class="py-12">
                    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                            {{ __('Edit Trucking Company') }}
                        </h2>
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6 text-gray-900">
                                <form action="{{ route('tracking_companies.update', $tracking_company) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <textarea id="note" name="note" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">{{ old('note', $tracking_company->note) }}</textarea>
                                    </div>
                                    <!-- Enabled/Disabled Toggle -->
                                    <div class="mb-6">
                                        <label class="inline-flex items-center cursor-pointer">
                                            <input type="checkbox" name="enabled" value="1" class="sr-only peer" {{ $tracking_company->enabled ? 'checked' : '' }}>
