<x-app-layout>
                    ...
                    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <div class="flex justify-between">
                            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                                {{ __('Stock Control Status') }}
                            </h2>
                            <a href="{{ route('stock_control_statuses.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 transition ease-in-out duration-150">Create stock_control_statuse</a>
                        </div>
                            ...
                            <div class="bg-white shadow-md rounded-lg p-6">
                                <table class="w-full table-auto">
                                    ...
                                    <tbody class="text-gray-700 text-sm font-light">
                                        @foreach($stock_control_statuses as $stock_control_statuse)
                                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                                <td class="py-3 px-6 text-left">
                                                    <span>{{ $stock_control_statuse->name }}</span>
                                                </td>
                                                <td class="py-3 px-6 text-left">
                                                    <span>{{ $stock_control_statuse->description }}</span>
                                                </td>
                                                <td class="py-3 px-6 text-left">
                                                    <span>{{ $stock_control_statuse->enabled ? 'True' : 'False' }}</span>
                                                </td>
                                                <td class="py-4 px-6 align-start text-center">
                                                    <div class="inline-flex items-center space-x-4">
                                                        <a href="{{ route('stock_control_statuses.edit', $stock_control_statuse) }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                        </svg>
                                                        </a>
                                                        <button type="button" onclick="openModal('{{ route('stock_control_statuses.destroy', $stock_control_statuse) }}')" class="text-gray-600 hover:text-gray-800">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <div id="deleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm mx-auto">
            ...
            <p class="text-gray-700 mb-6">Are you sure you want to delete this stock control status?</p>
